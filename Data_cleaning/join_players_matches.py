...
all_matches = pd.read_csv('cleaned_matches_data.csv', dtype=object, encoding='utf-8')
...
all_matches.drop(columns=to_drop_matches, inplace=True)
all_matches.drop(all_matches.columns[0], axis=1, inplace=True)

#Reclean invalid values that are not numeric in a given column
all_matches.replace(to_replace='NR',value=0,inplace=True)
all_matches.replace(to_replace=' ',value=0,inplace=True)
all_matches.replace(to_replace='`1',value=0,inplace=True)

#Convert names to uppercase 
all_matches['Winner'] = all_matches['Winner'].str.upper()
all_matches['Loser'] = all_matches['Loser'].str.upper()
...
all_matches.rename(columns=to_rename, inplace=True)

#Classification output winner
all_matches["PlayerA Win"] = '1'
...
index = all_matches.sample(frac=0.5,random_state=42).index
tmp = all_matches.loc[index,to_swapA].values
all_matches.loc[index,to_swapA] = all_matches.loc[index,to_swapB].values
all_matches.loc[index,to_swapB] = tmp
all_matches.loc[index,'PlayerA Win'] = 0
...
all_matches = pd.get_dummies(all_matches, columns = hot_encode_matches) 
...
#Creating a merge for every year
for year in all_matches['Year'].unique():
    matches = all_matches[all_matches['Year'] == str(year)]
...
# -----------------------------------------------DIFFERENCE---------------------------------------
    ...
    players_and_matches_diff.to_csv("training_data/training_matches_players_diff_{}.csv".format(year), index=False)
